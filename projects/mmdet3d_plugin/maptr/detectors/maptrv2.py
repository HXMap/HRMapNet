...
class MapTRv2(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        if self.update_map:
            new_map = self.pts_bbox_head.get_pred_mask(outs)
    ...
    def forward_train(self,
        ...
        if self.global_map is not None:
            ...
            local_map = self.obtain_global_map(img_metas, 'train')
        else:
            local_map = None

        losses_pts = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            gt_labels_3d, img_metas,
                                            gt_bboxes_ignore, local_map, gt_depth,gt_seg_mask,gt_pv_seg_mask)
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        if self.global_map is not None:
            self.global_map.check_map(img_feats[0].device, self.epoch, 'val')
            local_map = self.obtain_global_map(img_metas, 'val')
        else:
            local_map = None
