...
class InfiniteGroupEachSampleInBatchSampler(Sampler):
    ...
    Arguments:
        ...
        epoch_to_seq: After `num_iters_to_seq` iterations,
    ...
    def __init__(self,
        ...
        self.epoch_to_seq = epoch_to_seq
        assert hasattr(self.dataset, 'flag')
        self.flag = self.dataset.flag
        self.group_sizes = np.bincount(self.flag)
        ...
        self.last_group_idx_batch = [-1 for i in range(self.batch_size)]

        self.num_samples = int(math.ceil(self.size * 1.0 / self.batch_size /self.num_replicas)) * self.batch_size
    ...
    def _sample_sub_sequence(self, group_idx):
        ...
        while True:
            if self.epoch < self.epoch_to_seq or self.seq_split_num == -1:
    ...
    def __iter__(self):
        curr_batch = []
        print("get data index for epoch:", self.epoch)
        while True:
            for local_sample_idx in range(self.batch_size):
                if len(self.buffer_per_local_sample[local_sample_idx]) == 0:
                    ...
                    if new_group_idx == self.last_group_idx_batch[local_sample_idx]:
                        new_group_idx = next(self.group_indices_per_global_sample_idx[local_sample_idx])
                    self.last_group_idx_batch[local_sample_idx] = new_group_idx
            ...
            if len(curr_batch) == self.num_samples:
                return iter(curr_batch)
