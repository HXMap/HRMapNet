...
class CustomAV2OfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        lidar2global[:3, 3] = input_dict['ego2global_translation']
        lidar2global[:3, :3] = input_dict['ego2global_rotation']
    ...
    def _evaluate_single(self,
        ...
        for metric in metrics:
            ...
            for i, name in enumerate(self.MAPCLASSES):
                ...
                detail['AV2Map_{}/{}_AP'.format(metric,name)] = cls_aps1.mean(0)[i]
            print('map: {}'.format(cls_aps1.mean(0).mean()))
            detail['AV2Map_{}/mAP'.format(metric)] = cls_aps1.mean(0).mean()
            print('------------------------------')
            for i, name in enumerate(self.MAPCLASSES):
                print('{}: {}'.format(name, cls_aps2.mean(0)[i]))
                detail['AV2Map_{}/{}_AP'.format(metric,name)] = cls_aps2.mean(0)[i]
            print('map: {}'.format(cls_aps2.mean(0).mean()))
            detail['AV2Map_{}/mAP'.format(metric)] = cls_aps2.mean(0).mean()
