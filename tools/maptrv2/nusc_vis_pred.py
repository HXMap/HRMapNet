...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        for vis_format in args.gt_format:
            if vis_format == 'se_pts':
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_lines_bbox, gt_labels_3d[0]):
                    ...
                    # import pdb;pdb.set_trace()
                    plt.gca().add_patch(Rectangle(xy, width, height, linewidth=0.4, edgecolor=colors_plt[gt_label_3d],
                                                  facecolor='none'))
        ...
        plt.ylim(pc_range[1], pc_range[4])
        plt.axis('off')
        for pred_score_3d, pred_bbox_3d, pred_label_3d, pred_pts_3d in zip(scores_3d[keep], boxes_3d[keep],
                                                                           labels_3d[keep], pts_3d[keep]):
